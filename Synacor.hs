...
step m@Machine{..} =
    debugShow m $ case op of
        ...
        Halt -> debugOp [] $ return Nothing
        -- set: 1 a b
        --   set register <a> to the value of <b>
        Set -> do
            let [a, b] = take 2 args
            debugOp [a, b] $
                return $ Just (write a (v b)) { pc = pc + 3 }
        -- push: 2 a
        --   push <a> onto the stack
        Push -> do
            let [a] = take 1 args
            debugOp [a] $
                return $ Just (m{stack = v a:stack, pc = pc + 2})
        -- pop: 3 a
        ...
        Pop -> do
            let [a] = take 1 args
                s:ss = stack
            debugOp [a] $
                return $ Just (write a s) { stack = ss, pc = pc + 2 }
        -- eq: 4 a b c
        --   set <a> to 1 if <b> is equal to <c>; set it to 0 otherwise
        Eq -> do
            let [a, b, c] = take 3 args
            debugOp [a, b, c] $
                return $ Just (write a (if v b == v c then 1 else 0)) { pc = pc + 4 }
        -- gt: 5 a b c
        ...
        Jmp -> do
            let [a] = take 1 args
            debugOp [a] $
                return $ Just (m{pc = v a})
        -- jt: 7 a b
        --   if <a> is nonzero, jump to <b>
        Jt -> do
            let [a, b] = take 2 args
            debugOp [a, b] $
                return $ Just (m{pc = if v a /= 0 then v b else pc + 3})
        -- jf: 8 a b
        ...
        Add -> do
            let [a, b, c] = take 3 args
            debugOp [a, b, c] $
                return $ Just (write a ((v b + v c) `mod` 32768)) { pc = pc + 4 }
        -- mult: 10 a b c
        ...
        Mod -> do
            let [a, b, c] = take 3 args
            debugOp [a, b, c] $
                return $ Just (write a (v b `mod` v c)) { pc = pc + 4 }
        -- and: 12 a b c
        --   stores into <a> the bitwise and of <b> and <c>
        And -> do
            let [a, b, c] = take 3 args
            debugOp [a, b, c] $
                return $ Just (write a (v b .&. v c)) { pc = pc + 4 }
        -- or: 13 a b c
        ...
        Not -> do
            let [a, b] = take 2 args
            debugOp [a, b] $
                return $ Just (write a (complement (v b) `mod` 32768)) {pc = pc + 3}
        -- rmem: 15 a b
        --   read memory at address <b> and write it to <a>
        Rmem -> do
            let [a, b] = take 2 args
            debugOp [a, b] $ return $ Just (write a (read (v b))) { pc = pc + 3 }
        -- wmem: 16 a b
        ...
        Call -> do
            let [a] = take 1 args
            debugOp [a] $
                return $ Just (m{stack = (pc + 2):stack, pc = v a})
        -- ret: 18
        ...
        In -> do
            ...
            b <- getChar
            debugOp [a] $
                return $ Just (write a (fromIntegral (ord b))) { pc = pc + 2 }
